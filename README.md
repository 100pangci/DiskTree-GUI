...
- `ubuntu-latest`：构建 Android Debug 包与 Linux Desktop 发行产物
- `macos-latest`：构建 macOS Desktop 发行产物
...
- 当前仓库会在 GitHub Actions 中单独拉取该文件进行构建
- 然后使用 `PyInstaller` 打包为单文件可执行程序
- `Tree` 不会混入 Desktop GUI 的压缩包/目录产物，而是作为单独构建产物和单独 Release 资源上传
...
- `releases-tree-linux-*`
- `releases-tree-macos-*`
- `releases-tree-windows-*`

其中 GUI 桌面产物与 `Tree` 可执行文件是分开上传的；`Tree` 会以独立 artifact / Release 资源的形式提供，而不会放进 GUI 桌面压缩包里。